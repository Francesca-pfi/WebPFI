...
class Album{
    ...
    public function display_album(){
        ...
        $nbLikes = $TDG->get_number_of_likes($this->id, "album");
        ...
        if (validate_session()) {
            ...
            if ($TDG->is_elem_liked_by($this->id, "album", $_SESSION["userID"])){
                $btnLike = 
    ...
    public static function create_album_list($TDG_res){
        $albums_list = array();
        foreach ($TDG_res as $alb) {
            $album = new Album();

            $album->load_album($alb["id"]);
    ...
    public static function add_album($title,$descr,$userID){
        ...
        $resp = $TDG->add_album($title,$descr,$userID,$date->format('Y-m-d H:i:s'));
        $TDG = null;
        return $resp;
    }
